...
export const searchInterviewsSchema = {
    ...
    description: 'Pesquisa entrevistas NPS com filtros por texto, região, sentimento, classificação NPS, segmento e campanha. ' +
...
export async function searchInterviews(args) {
    const input = SearchInterviewsInput.parse(args);
    const where = ['v.tenant_id = $1'];
    const params = [input.tenant_id];
    ...
    if (input.query) {
        ...
        where.push(`(v.respondent_name ILIKE $${idx} OR v.summary_text ILIKE $${idx} OR v.topics_json::text ILIKE $${idx})`);
    }
...
export const getInterviewDetailSchema = {
    ...
    description: 'Retorna detalhes completos de uma entrevista específica, incluindo respostas individuais, ' +
...
export async function getInterviewDetail(args) {
...
export const getNpsSummarySchema = {
    ...
    description: 'Retorna resumo estatístico do NPS: score geral, distribuição (promotores, neutros, detratores), ' +
...
export async function getNpsSummary(args) {
...
export const searchTopicsSchema = {
    name: 'search_topics',
    description: 'Pesquisa tópicos mencionados nas entrevistas. Mostra frequência de cada tópico e permite filtrar por campanha. ' +
...
export const getDetractorInsightsSchema = {
    ...
    description: 'Análise focada nos detratores: lista detratores recentes com drivers negativos, ' +
...
export async function getDetractorInsights(args) {
    const input = GetDetractorInsightsInput.parse(args);
    const where = ['e.tenant_id = $1', "e.nps_class = 'detractor'"];
    const params = [input.tenant_id];
    if (input.campaign_id) {
        params.push(input.campaign_id);
        where.push(`e.campaign_id = $${params.length}`);
