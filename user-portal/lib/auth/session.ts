...
export function saveSession(session: AuthSession): void {
  ...
  document.cookie = `${SESSION_COOKIE_NAME}=${encodeURIComponent(encoded)}; Path=/; Max-Age=${maxAge}; SameSite=Lax`;
}
...
export function readSession(): AuthSession | null {
  ...
  if (cookieValue) {
    const fromCookie = decodeSession(decodeURIComponent(cookieValue));
